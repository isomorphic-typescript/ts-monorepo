...
import { GLOBAL_SCOPE_NAME, Success, SUCCESS, CONFIG_FILE_NAME, PACKAGES_DIRECTORY_NAME } from '../../common/constants';
...
export function validateScope(scope: string): Either<ConfigError[], Success> {
    ...
    if (scope !== GLOBAL_SCOPE_NAME) {
        const invalidScopePrefix = `Keys of ${colorize.file(CONFIG_FILE_NAME)}[${PACKAGES_DIRECTORY_NAME}] must be npm scopes.\nScope "${colorize.scope(scope)}" is not a valid scope.`;
